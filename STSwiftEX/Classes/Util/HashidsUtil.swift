...
public class HashidsUtil{
    public static func uidHashids(uid:Int64) ->String{
        let millSecond = Int64(NSDate().timeIntervalSince1970*1000)
        print("\(uid),\(millSecond)")
        let hashids = Hashids(salt:String(Date().timeIntervalSinceNow), minHashLength:8, alphabet:nil);
        return hashids.encode([uid,millSecond])!
    }
}
